...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.Feat_Encoder = nn.Sequential(*(
                ...
                models.resnet18(weights='ResNet18_Weights.DEFAULT').children())[1:-2]))
    ...
    def forward(self, ST, QR, QRs=None, mode='train'):
        ...
        if (not IS_CYCLE) and IS_KLD:
            ...
            OUT_Feats1_mu = torch.cat(OUT_Feats1_mu, 1);
            OUT_Feats1_logvar = torch.cat(OUT_Feats1_logvar, 1);
            ...
            def _get_lda(Ex_mu, Dx_mu, Ex_logvar, Dx_logvar):
                return torch.sqrt(torch.sum((Ex_mu - Dx_mu) ** 2, dim=1) + \
                                  torch.sum((torch.sqrt(Ex_logvar.exp()) - torch.sqrt(Dx_logvar.exp())) ** 2,
                                            dim=1)).sum()

            lda1 = [_get_lda(memory_mu[:, idi, :], OUT_Feats1_mu[:, idj, :], memory_logvar[:, idi, :],
                             OUT_Feats1_logvar[:, idj, :]) for idi in range(memory.shape[0]) for idj in
                    range(OUT_Feats1.shape[0])]
        ...
        with torch.no_grad():
            ...
            if IS_SEQ:
                ...
                FEAT_ST_T = self.Feat_Encoder(torch.cat([torch.cat(
                    [i_, torch.ones((i_.shape[0], i_.shape[1], i_.shape[2], max_width_ - i_.shape[3])).to(DEVICE)], -1)
                                                         for i_ in OUT_IMGS], 1))
        ...
        Lcycle1 = np.sum([self.l1loss(memory[m_i], memory_T[m_j]) for m_i in range(memory.shape[0]) for m_j in
                          range(memory_T.shape[0])]) / (memory.shape[0] * memory_T.shape[0])
        OUT_Feats1 = torch.cat(OUT_Feats1, 1)[0];
        OUT_Feats2 = torch.cat(OUT_Feats2, 1)[0]

        Lcycle2 = np.sum([self.l1loss(OUT_Feats1[f_i], OUT_Feats2[f_j]) for f_i in range(OUT_Feats1.shape[0]) for f_j in
                          range(OUT_Feats2.shape[0])]) / (OUT_Feats1.shape[0] * OUT_Feats2.shape[0])

        if IS_KLD:
            OUT_Feats1_mu = torch.cat(OUT_Feats1_mu, 1);
            OUT_Feats1_logvar = torch.cat(OUT_Feats1_logvar, 1);
            OUT_Feats2_mu = torch.cat(OUT_Feats2_mu, 1);
            OUT_Feats2_logvar = torch.cat(OUT_Feats2_logvar, 1);
            ...
            lda1 = [_get_lda(memory_mu[:, idi, :], OUT_Feats1_mu[:, idj, :], memory_logvar[:, idi, :],
                             OUT_Feats1_logvar[:, idj, :]) for idi in range(memory.shape[0]) for idj in
                    range(OUT_Feats1.shape[0])]
            lda2 = [_get_lda(memory_T_mu[:, idi, :], OUT_Feats2_mu[:, idj, :], memory_T_logvar[:, idi, :],
                             OUT_Feats2_logvar[:, idj, :]) for idi in range(memory_T.shape[0]) for idj in
                    range(OUT_Feats2.shape[0])]
...
class TRGAN(nn.Module):
    ...
    def visualize_images(self):
        ...
        imgs['res-1'] = torch.cat(
            [self.sdata[0, 0], self.sdata[0, 1], self.sdata[0, 2], imgs['fake-1'], imgs['fake-2'], imgs['fake-3']], -1)
        ...
        imgs['res-2'] = torch.cat(
            [self.sdata[1, 0], self.sdata[1, 1], self.sdata[1, 2], imgs['fake-1'], imgs['fake-2'], imgs['fake-3']], -1)
        ...
        imgs['res-3'] = torch.cat(
            [self.sdata[2, 0], self.sdata[2, 1], self.sdata[2, 2], imgs['fake-1'], imgs['fake-2'], imgs['fake-3']], -1)
    ...
    def forward(self):
        ...
        if IS_CYCLE and IS_KLD:
            ...
            self.fake, self.Lcycle1, self.Lcycle2, self.lda1, self.lda2, self.KLD = self.netG(self.sdata,
                                                                                              self.text_encode_fake,
                                                                                              self.text_encode_fake_js)

        elif IS_CYCLE and (not IS_KLD):

            self.fake, self.Lcycle1, self.Lcycle2 = self.netG(self.sdata, self.text_encode_fake,
                                                              self.text_encode_fake_js)
