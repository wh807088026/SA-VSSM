...
def print_shape(variable, temp='', is_tensor=False):
    """
    打印变量的形状信息

    参数:
        variable: 待打印形状信息的变量
        is_tensor: 张量的内容是否打印，默认为 False
    """
    try:
        if hasattr(variable, 'shape'):
            info_str = '{} {}.shape: {}'.format(temp, argname('variable'), variable.shape)
            if is_tensor and isinstance(variable, torch.Tensor):
                info_str += ', {} = {}'.format(argname('variable'), variable)
        elif isinstance(variable, list):
            info_str = '{} {}.len: {}'.format(temp, argname('variable'), len(variable))
        else:
            info_str = '{} {}.shape: {}'.format(temp, argname('variable'), 'no attribute shape')
    except Exception as e:
        info_str = 'Error: {}'.format(str(e))
    print(info_str)


class StyleAdaptiveNorm(nn.Module):
    def __init__(self, normalized_shape, style_dim=512, eps=1e-6):
        super().__init__()
        self.eps = eps
        self.normalized_shape = normalized_shape

        # 原本 LayerNorm 的 weight/bias 是可学习参数
        # 现在我们要用 Style 生成它们
        # 不需要 self.weight 和 self.bias

        # 风格投影层: Style -> (Scale, Shift)
        # Scale 控制方差(笔画粗细/对比度)，Shift 控制均值(背景/基调)
        self.style_proj = nn.Linear(style_dim, normalized_shape * 2)

        # 初始化：确保初始状态 Scale=1, Shift=0 (即表现得像普通 Norm)
        self.style_proj.weight.data.fill_(0)
        self.style_proj.bias.data[:normalized_shape] = 1.0  # Scale init 1
        self.style_proj.bias.data[normalized_shape:] = 0.0  # Shift init 0
    ...
    def forward(self, x, style):
        ...
        style: [B, Style_Dim]
        """
        # 1. 计算标准 LayerNorm (不带仿射变换)
        # x 是 channel first，需要 permute 计算统计量
...
class SS2Dv2:
    def __initv2__(
        ...
        self.in_proj2 = Linear(d_model, d_proj, bias=bias, **factory_kwargs)
        self.act: nn.Module = act_layer()
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        """
            Args:
                x.shape: torch.Size([8, 512, 1, 16])
                kwargs:
            Param:
                linear: x.shape: torch.Size([8, 1024, 1, 16])
                DWConv: x.shape: torch.Size([8, 1024, 1, 16])
                Silu: x.shape: torch.Size([8, 1024, 1, 16])
                SS2D: x.shape: torch.Size([8, 1024, 1, 16])
                LN: x.shape: torch.Size([8, 1024, 1, 16])
                linear: x.shape: torch.Size([8, 1024, 1, 16])
            Returns:
                out.shape: torch.Size([8, 512, 1, 16])
            Notes:
        """
        # print("forwardv2")
        ...
        # linear
        if not self.disable_z:  # True
            x1, z = x1.chunk(2, dim=(1 if self.channel_first else -1))  # (b, h, w, d)
            if not self.disable_z_act:  # False
                z = self.act(z)
        if not self.channel_first:  # True
...
class VSSBlock(nn.Module):
    def __init__(
            ...
            transformerDecoder: bool = False,

            style_dim=512,  # 接收风格维度
            use_adain=True,  # 开关
            **kwargs,
    ):
        super().__init__()
        ...
        self.skip_scale = nn.Parameter(torch.ones(hidden_dim, 1, 1))
        self.skip_scale1 = nn.Parameter(torch.ones(hidden_dim, 1, 1))
        self.skip_scale2 = nn.Parameter(torch.ones(hidden_dim, 1, 1))
        if self.ssm_branch:
            if self.use_adain and self.ssm_branch:
                # 替换 self.norm
                self.norm = StyleAdaptiveNorm(hidden_dim, style_dim=style_dim)
            else:
        ...
        if self.transformerDecoder:
            self.decoder_layer = CrossLayer(512, 8, 0.1)

    def _forward(self, input, input2):
        """
            Args:
                input (type): torch.Size([8, 512, 1, 16])
                input2 (type): torch.Size([8, 512, 1, 90])
            Returns:
                x (type): torch.Size([8, 512, 1, 16])
        """
        x = input
        y = input2  # y 是 Style 特征图 [B, Dim, 1, W]

        # 1. 提取全局风格向量 (用于 AdaIN)
        # 即使不开 transformerDecoder，AdaIN 也需要这个全局向量
        style_vec = None
        if self.use_adain:
            # 全局平均池化: [B, 512, 1, 90] -> [B, 512]
            # flatten(2) 将后面维度展平，mean(2) 在展平后的维度取平均
            style_vec = y.flatten(2).mean(2)

            # 2. 特征融合分支 (Cross-Attention)
        if self.transformerDecoder:
            if self.post_norm:
                # 注意：这里 decoder_layer 是 Transformer，通常自带 Norm，这里 self.norm 是给 SSM 用的
                # 如果这里的 self.norm 变成了 StyleAdaptiveNorm，则调用方式需改变
                # 但根据原代码逻辑，decoder_layer 前面有一个 self.norm
                # 如果 use_adain 开启，这里的 self.norm 是 StyleAdaptiveNorm，需要传 style_vec

                # 为了安全起见，通常 TransformerDecoder 分支还是用标准 Norm
                # 或者复用 AdaIN。这里假设 TransformerDecoder 自带 Norm 或者不共用 self.norm
                # 原代码逻辑中 self.norm 是定义在 ssm_branch 里的
                # 如果 transformerDecoder 用的是 ssm_branch 定义的 norm:

                if self.use_adain:
                    x = x * self.skip_scale + self.drop_path(self.norm(self.decoder_layer(x), style_vec))
                else:
                    x = x * self.skip_scale + self.drop_path(self.norm(self.decoder_layer(x)))
            else:
                # Pre-Norm 逻辑
                FEAT_x = x.flatten(2).permute(2, 0, 1)
                FEAT_y = y.flatten(2).permute(2, 0, 1)
                tgt = torch.zeros_like(FEAT_x)

                # decoder_layer forward
                memory = self.decoder_layer(tgt, FEAT_y, query_pos=FEAT_x)
                memory = self.drop_path(memory).unsqueeze(0).permute(2, 3, 0, 1)

                x = x * self.skip_scale + memory

        # 3. SSM 分支 (核心修改处)
        if self.ssm_branch:
            if self.post_norm:
                # Post Norm: 先过 SSM，再过 Norm
                output = self.op(x)
                if self.use_adain:
                    # 使用 Style 进行自适应归一化
                    x = x * self.skip_scale1 + self.drop_path(self.norm(output, style_vec))
                else:
                    x = x * self.skip_scale1 + self.drop_path(self.norm(output))
            else:
                # Pre Norm: 先过 Norm，再过 SSM (这是 VSSM 的标准做法)
                if self.use_adain:
                    # 先根据 Style 调整分布 (AdaIN)，再送入 Mamba
                    # 这样 Mamba 接收到的就是已经被"风格化"的数据分布
                    x_normed = self.norm(x, style_vec)
                    x = x * self.skip_scale1 + self.drop_path(self.op(x_normed))
                else:
                    x = x * self.skip_scale1 + self.drop_path(self.op(self.norm(x)))

        # 4. MLP 分支 (保持不变)
        if self.mlp_branch:
            if self.post_norm:
                x = x * self.skip_scale2 + self.drop_path(self.norm2(self.mlp(x)))
            else:
                x = x * self.skip_scale2 + self.drop_path(self.mlp(self.norm2(x)))
    ...
    def forward(self, input, input2=None):
        ...
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input, input2)
        else:
            return self._forward(input, input2)
...
class VSSM(nn.Module):
    ...
    def _make_layer(
        ...
        return SequentialWithArgs(*blocks)

    def forward(self, x, y=None):
        """
            Args:
                x (type): torch.Size([8, 15, 32, 192])
                y :
            Param:
                output1 (type): torch.Size([8, 64, 8, 48])
                output2 (type): torch.Size([8, 128, 4, 24])
                output3 (type): torch.Size([8, 256, 2, 12])
                output4 (type): torch.Size([8, 512, 1, 6])

            Returns:
                memory (type): torch.Size([8, 512, 1, 6])
            Notes:
    ...
    def flops(self, shape=(3, 224, 224)):
        ...
        supported_ops = {
            ...
            # "prim:a:PythonOp.CrossScan": None,
